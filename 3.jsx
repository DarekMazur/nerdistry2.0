...
Every menu item should be displayed inside a separate corresponding div. This div should have a dynamically created attribute, data-test-id, in the form: first-level-{lowercase-title-name-here}. So, for example, if some MenuItem
contained a title named Home, the div should have a data-test-id containing first-level-home.
...
button with a dynamic data-test-id in the form: button-{lowercase-title-name-here} (for example, button-home). The button
should only be displayed when there are subItems for the given menu item. When clicking the button, the submenu with subItems (the ul list described in the next point) should appear (be added to the DOM) if it is hidden, and should disappear (be removed from the DOM) if it is expanded. The text inside the button should be Expand when the given
menu is not expanded, and Hide if the menu has already been expanded
...
li tags inside the aforementioned ul tag. Each li should have a data-test-id in the form: li-{lowercase-title-name-here}-{lowercase-subitem-name-here}. So, for example, for the Home title and the subItems Main and Services, the li data-test-ids should be
li-home-main and li-home-services. Inside every li tag there should be a subItem name (taken from the array subItems from the corresponding MenuItem). The display state of these submenu lists is controlled by the aforementioned corresponding buttons
...
function Solution({ menuConfig }: { menuConfig: MenuConfig }): ReactElement {
  return <div className="menu-wrapper">Implement your solution here</div>;
}
